...
const Sidebar = () => {
  ...
  useEffect(()=>{
    ...
    if(category == "" && (pricevalue[0]!="100" || pricevalue[1]!="1000")){
      ...
      let totaldata = data.products;
      let filterdata = totaldata.filter((item) => item.price<=pricevalue[1] && item.price>=pricevalue[0]);
    ...
    let filterdata = totaldata.filter((item) => item.category == filtervalue && item.price<=pricevalue[1] && item.price>=pricevalue[0]);
    dispatch((getProduct({filterdata:filterdata,arrayvalues:pricevalue})));
    }
    // else{
    //   let totaldata = data.products;
  ...
  return (
    <>
      <Drawer
        ...
        anchor="left"
      >
        <Typography gutterBottom variant="h3" color="black">Filter :</Typography>
        
        {category!=="" &&
        <Chip label={category}  onDelete={handleDelete} sx={{width:"80%",fontSize:"17px"}} />}
        
       {pricelabel !== "" &&
        <Chip  label={pricelabel} onDelete={handlePriceDelete} sx={{width:"80%",marginTop:"15px",fontSize:"17px"}} />}
        ...
        {categories &&
            ...
            categories.map((item) => {
              return (
                <li className="sidebar__categoryitems" onClick={()=>handleCategoryChange(item)}>
                  {item}
                </li>
              );
            })}
